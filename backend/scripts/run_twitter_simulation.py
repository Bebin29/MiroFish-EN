...
This script reads parameters from the configuration file and executes a fully automated simulation.

Features:
- After the simulation completes, keep the environment alive and enter command-wait mode.
...
# Load the .env file from project root (contains LLM_API_KEY and related settings)
...
class MaxTokensWarningFilter(logging.Filter):
    """Filter camel-ai max_tokens warnings (we intentionally do not set max_tokens)."""
    
    def filter(self, record):
        # Filter log messages that contain max_tokens warnings
...
class IPCHandler:
    ...
    def poll_command(self) -> Optional[Dict[str, Any]]:
        """Poll and fetch pending commands."""
...
class TwitterSimulationRunner:
    ...
    # Available Twitter actions (INTERVIEW is excluded and can only be triggered via ManualAction)
    AVAILABLE_ACTIONS = [
    ...
    def _create_model(self):
        ...
        Use settings from project root .env file with highest priority:
        - LLM_API_KEY: API key
        ...
        # If missing in .env, use config as fallback
        if not llm_model:
    ...
    def _get_active_agents_for_round(
        ...
        for cfg in agent_configs:
            ...
            # Check whether current time is in active hours
...
async def main():
    ...
    # Initialize logging config (fixed file names, clean old logs)
...
def setup_signal_handlers():
    """
    Set signal handlers to ensure graceful exit on SIGTERM/SIGINT.
    This gives the program a chance to clean up resources (database, environment, etc.).
    """
