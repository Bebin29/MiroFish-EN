...
Run Twitter and Reddit simulations in parallel with the same config file

Features:
- Dual-platform (Twitter + Reddit) parallel simulation
- Do not close environments immediately after simulation; enter command-wait mode
...
# This fixes third-party OASIS file reads where encoding is not specified
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    # Reconfigure stdout/stderr to UTF-8 (fix console garbled text)
    ...
    # Note: this should be set at Python startup; runtime changes may not fully apply
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        This fixes third-party libs (like OASIS) reading files without encoding
        """
        # Set default encoding only for text mode (non-binary) when encoding is not specified
...
# Script is located under backend/scripts/
...
# Load .env from project root (includes LLM_API_KEY, etc.)
...
class MaxTokensWarningFilter(logging.Filter):
    """Filter camel-ai max_tokens warnings (intentionally not setting max_tokens)"""
...
def disable_oasis_logging():
    """
    Disable verbose OASIS logging output
    OASIS logs are too verbose; we use our own action_logger
    """
...
def init_logging_for_simulation(simulation_dir: str):
    ...
    # Clean old log directory (if present)
...
# Available Twitter actions (excluding INTERVIEW; trigger manually via ManualAction)
TWITTER_ACTIONS = [
...
# Available Reddit actions (excluding INTERVIEW; trigger manually via ManualAction)
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
...
class ParallelIPCHandler:
    ...
    Manage both platform environments and handle Interview commands
    """
    ...
    def _get_env_and_graph(self, platform: str):
        """
        Get env and agent_graph for the specified platform
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Specified platform (optional)
                ...
                - None/not specified: interview both platforms and return merged result
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        twitter_interviews = []
        reddit_interviews = []
        both_platforms_interviews = []  # requires both-platform interviews
...
# Non-core action types to filter (low analysis value)
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    Show real entity names in actions.jsonl instead of aliases like "Agent_0"
...
def fetch_new_actions_from_db(
    ...
    """
    Fetch new actions from DB and enrich context
    
    Args:
        db_path: DB file path
        last_rowid: Last read max rowid (use rowid instead of created_at because created_at formats differ by platform)
        agent_names: agent_id -> agent_name mapping
        
    Returns:
        (actions_list, new_last_rowid)
        - actions_list: Action list; each item includes agent_id, agent_name, action_type, action_args (with context)
    ...
    try:
        ...
        # Use rowid to track processed records (SQLite built-in auto-increment field)
        # Avoid created_at format differences (Twitter int, Reddit datetime string)
        cursor.execute("""
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Simplify action_args and keep key fields (full content, no truncation)
            simplified_args = {}
...
def _enrich_action_context(
    ...
    """
    Enrich actions with context (post content, usernames, etc.)
    
    Args:
        cursor: Database cursor
        action_type: Action type
        action_args: Action args (mutated)
        agent_names: agent_id -> agent_name mapping
    """
    try:
        ...
        # Repost: enrich original post content and author
            ...
            if new_post_id:
                # For reposts, original_post_id points to original post
                cursor.execute("""
        ...
        # Quote post: enrich original content, author, and quote content
            ...
            # Get quote content (quote_content) from quoted post
            if new_post_id:
                cursor.execute("""
        ...
        # Follow user: enrich followed user name
        ...
        # Mute user: enrich muted user name
        ...
        # Create comment: enrich target post info
        ...
        # Context enrichment failure should not break main flow
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    Supports dual LLM config to speed up parallel simulation：
    ...
    With boost LLM configured, different platforms can use different API providers to improve concurrency.
    ...
    # If model name missing in .env, use config as fallback
    if not llm_model:
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        raise ValueError("Missing API Key config; set LLM_API_KEY in project root .env")
...
def get_active_agents_for_round(
    ...
    """Determine active agents this round by time and config"""
...
async def run_twitter_simulation(
    ...
    # Twitter Use general LLM config
    ...
    # If an agent is missing in config, use OASIS default name
    ...
    result.env = oasis.make(
        ...
        semaphore=30,  # Limit max concurrent LLM calls to avoid API overload
    )
    ...
    total_actions = 0
    last_rowid = 0  # Track last processed DB row (use rowid to avoid created_at format differences)
...
async def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Close environments immediately after simulation; do not enter command-wait mode'
    )
    
    args = parser.parse_args()
    
    # Create shutdown event at start of main to ensure full-program signal response
    ...
    if args.twitter_only:
        ...
        # Run in parallel (independent logger per platform)
    ...
    if wait_for_commands:
        log_manager.info("")
        log_manager.info("=" * 60)
        log_manager.info("Entering command-wait mode - environments stay running")
...
def setup_signal_handlers(loop=None):
    ...
    Persistent simulation mode: after simulation, wait for interview commands
    When termination signal is received, we need to:
    1. Notify asyncio loop to exit waiting state
    2. Allow program to clean resources normally (DB, env, etc.)
    3. then exit
    """
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Set event to notify asyncio loop to exit (allow cleanup)
            if _shutdown_event:
                _shutdown_event.set()
        
        # Do not call sys.exit() directly; let asyncio loop cleanly exit
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Clean multiprocessing resource tracker (prevent exit warning)
        try:
